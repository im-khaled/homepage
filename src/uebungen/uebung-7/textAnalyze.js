const textAnalysis = (inputText) => {
    ...
    // Replacing HTML tags into empty String
    ...
    // Sorting the map by word frequency 
    const sortedWordCount = new Map([...wordCountMap.entries()].sort((a, b) => b[1] - a[1]));
  
    // Reduzierung das Ergebnis auf die 3 häufigsten Begriffe.
    const topThreeWords = [...sortedWordCount.entries()].slice(0, 3).reduce((acc, [word]) => {
      acc.push(word);
      return acc;
    }, []);
...
fetch(Url)
  .then(res => res.text())
  .then(textResult => {
    const result = textAnalysis(textResult);
    console.log(result);
  })
  .catch(error => {
